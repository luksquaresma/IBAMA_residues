...
import json, multiprocessing, requests, warnings
import pandas as pd
...
def lambda_handler(event, context):
    if event["source"] == "IBAMA":
        try:
            ...
            return {
                "statusCode": 200,
                "body": json.dumps("EXTRACTION OK")
            }
        except:
            return {
                "statusCode": 500,
                "body": json.dumps("ERROR ON EXTRACTION")
            }
    else:
        return {
            "statusCode": 500,
            "body": json.dumps("ERROR ON SOURCE")
        }
